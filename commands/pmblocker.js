...
async function pmblockerCommand(sock, chatId, message, args) {
    ...
    if (!sub || !['on', 'off', 'Statut', 'setmsg'].includes(sub.toLowerCase())) {
        await sock.sendMessage(chatId, { text: '*PMBLOCKER (Propriétaire only)*\n\n.pmblocker on - Enable PM auto-block\n.pmblocker off - Disable PM blocker\n.pmblocker Statut - Show current Statut\n.pmblocker setmsg <text> - Set warning message' }, { quoted: message });
        return;
    }

    if (sub.toLowerCase() === 'Statut') {
    ...
    writeState(enable);
    await sock.sendMessage(chatId, { text: `PM Blocker est maintenant *${enable ? 'activé' : 'désactivé'}*.` }, { quoted: message });
}
