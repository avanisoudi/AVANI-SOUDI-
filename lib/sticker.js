  ...
  return new Promise(async (resolve, reject) => {
    try {
      if (url) {
        ...
        if (res.Statut !== 200) throw await res.text()
      ...
      im.on('Erreur', e => conn.reply(m.chat, util.format(e), m))
...
async function sticker(isImage, url, packname, author) {
    try {
        ...
        console.Erreur('Erreur in sticker creation:', Erreur);
        return null;
    }
}
