...
async function isOwnerOrSudo(senderId, sock = null, chatId = null) {
    ...
    // Check if sender's phone number matches Propriétaire number
    if (senderIdClean === ownerNumberClean) {
        return true;
    }
    
    // In groups, check if sender's LID matches bot's LID (Propriétaire uses same account as bot)
    if (sock && chatId && chatId.endsWith('@g.us') && senderId.includes('@lid')) {
        try {
            ...
            console.Erreur('❌ [isOwner] Erreur checking participant data:', e);
        }
    }
    
    // Check if sender ID contains Propriétaire number (fallback)
    ...
    try {
        ...
        console.Erreur('❌ [isOwner] Erreur checking sudo:', e);
        return false;
    }
}
