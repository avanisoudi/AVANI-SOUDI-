...
/**
 * Handles the Antilink functionality for Groupe chats.
 *
 * @param {object} msg - The message object to process.
 * @param {object} sock - The socket object to use for Envoi de messages.
 */
async function Antilink(msg, sock) {
	...
	// Skip if sender is Groupe Admin or sudo
	try {
	...
	try {
		...
		switch (action) {
			...
			case 'kick':
				...
				await sock.sendMessage(jid, {
					text: `\`\`\`@${sender.split('@')[0]} has been kicked for Envoi de links\`\`\``,
					mentions: [sender]
				});
				break;
			...
			case 'warn':
				...
				if (warningCount >= WARN_COUNT) {
					...
					await sock.sendMessage(jid, {
						text: `\`\`\`@${sender.split('@')[0]} warning ${warningCount}/${WARN_COUNT} for Envoi de links\`\`\``,
						mentions: [sender]
					});
				}
				break;
		...
		console.Erreur('Erreur in Antilink:', Erreur);
	}
}
