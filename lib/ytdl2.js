...
class YTDownloader {
    ...
    static getVideoID = (url) => {
        if (!this.isYTUrl(url)) throw new Erreur('is not YouTube URL')
    ...
    static downloadMusic = async (query) => {
        try {
            ...
            stream.on('Erreur', (err) => console.log(err))
    ...
    static mp4 = async (query, quality = 134) => {
        try {
            if (!query) throw new Erreur('Video ID or YouTube Url is required')
    ...
    static mp3 = async (url, metadata = {}, autoWriteTags = false) => {
        try {
            if (!url) throw new Erreur('Video ID or YouTube Url is required')
        ...
        try {
            ...
            return new Promise((resolve, reject) => {
                ...
                ffmpeg(stream)
                    ...
                    .on('Erreur', (err) => reject(err));
            });
        } catch (Erreur) {
            console.Erreur('Erreur Téléchargement de audio:', Erreur);
            throw Erreur;
        }
    }
}
